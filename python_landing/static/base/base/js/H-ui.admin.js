...
/*用户-查看*/
function user_show(w,h,title,url){
	layer_show(w,h,title,url);
}
/*用户-密码-修改*/
...
/*用户-编辑*/
function user_edit(w,h,title,url){
	layer_show(w,h,title,url);
}
/*用户-编辑-保存*/
...
/*课程-编辑*/
function curriculum_edit(w,h,title,url){
	layer_show(w,h,title,url);
}

/*课程-删除*/
function curriculum_del(obj,id,url){
	layer.confirm('确认要删除吗？',function(index){
		curriculum_del_Ajax(id,url);
		$(obj).parents("tr").remove();
		layer.msg('已删除!',1);
	});
}

function curriculum_del_Ajax(id, url) {
	$.ajax({
		type : "post",
		url : url,
		dataType : "text",
...
function admin_stop(obj,id,url){
	admin_change_state_Ajax(id,url,0)
...
function admin_start(obj,id,url){
	admin_change_state_Ajax(id,url,1);
...
/*管理员 启用 禁用 执行的Ajax*/
function admin_change_state_Ajax(id, url, state) {
	$.ajax({
		type : "post",
		url : url,
		dataType : "text",
		data : {
			"tag" : "change_start", // 操作数
			"userId" : id,
			"state" : state
		},
		success : function(Result) {
